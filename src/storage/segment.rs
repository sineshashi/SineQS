...
impl Segment {
    ...
    pub async fn new(segment_id: i64, folder: String, active: bool) -> Self {
        let write_handler;
    ...
    pub async fn add_message(&mut self, message: Message) -> Result<i32, MessageIOError> {
    ...
    pub async fn read_message(&self, offset: i32) -> Result<Message, MessageIOError> {
        ...
        let mut buf = [0u8; 4];
        let _ = file.seek(io::SeekFrom::Start(offset as u64)).await?;
        let n = file.read(&mut buf).await;
        let message_len = i32::from_le_bytes(buf);
        let mut message_bytes = vec![0u8; message_len as usize];
        let _ = file.seek(io::SeekFrom::Start(offset as u64 + 4)).await?;
        let n = file.read(&mut message_bytes).await;
        return Ok(Message {
...
impl SegmentManager {
    pub async fn new(
        ...
        let segment_index =
            SegmentIndex::new(segment_id, String::from(&folder), active, offset_type).await;
        let cnt_index_page;
        if active {
            cnt_index_page = Some(
                segment_index
                    .get_cnt_page_being_written(max_number_of_records_in_index_page).await
        ...
        return Self {
            segment: Segment::new(segment_id, String::from(&folder), active).await,
            segment_index: segment_index,
            cnt_index_page: cnt_index_page,
        };
    }

    pub async fn get_last_written_segment_in_the_page(&self) -> Option<SegmentOffset> {
        match &self.cnt_index_page {
            Some(val) => val.get_last_written_segment().await,
            None => None,
        }
    }

    pub async fn deactivate(&mut self) {
        self.segment.deactivate().await;
        self.segment_index.deactivate().await;
    ...
    pub async fn add_message(
        ...
        let physical_offset = self.segment.add_message(message).await?;
        ...
        let res = self
            ...
            .as_mut()
            .unwrap()
            .add_segment_offset(&offset).await;
        if res.is_none() {
        ...
        self.segment_index
            .write_page(&self.cnt_index_page.as_ref().unwrap()).await?;
    ...
    pub async fn get_message(
        ...
        for poffset in page_offsets {
            ...
            let offset = page.unwrap().get_segment_offset(message_offset.clone()).await;
            if offset.as_ref().is_some() {
                return Ok(Some(
                    self.segment.read_message(offset.unwrap().physical_offset).await?,
                ));
            }
            set.insert(poffset);
        }
        ...
        for i in (0..=((number_of_bytes as f64 / page_size as f64).ceil() as i32)).rev() {
            ...
            let offset = page.get_segment_offset(message_offset.clone()).await;
            if offset.as_ref().is_some() {
                return Ok(Some(
                    self.segment.read_message(offset.unwrap().physical_offset).await?,
                ));
